...
```bash
docker pull sameersbn/gitlab-ci:7.14.0
```
...
You need to provide the URL of the GitLab server while running GitLab CI using the `GITLAB_URL` environment configuration. Since version `5.4.0` you need to provide the `GITLAB_APP_ID` and `GITLAB_APP_SECRET`. Since `7.14.0` you also need to provide the `GITLAB_CI_SECRETS_SESSION_KEY_BASE` and `GITLAB_CI_SECRETS_DB_KEY_BASE` parameters.
...
docker run --name gitlab-ci -it --rm [OPTIONS] \
    sameersbn/gitlab-ci:7.14.0 app:rake backup:create
```
...
```bash
docker run --name gitlab-ci -it --rm [OPTIONS] \
    sameersbn/gitlab-ci:7.14.0 app:rake backup:restore
```
...
```bash
docker run --name gitlab-ci -it --rm [OPTIONS] \
    sameersbn/gitlab-ci:7.14.0 app:rake backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab-ci -d [OPTIONS] sameersbn/gitlab-ci:7.14.0
```
