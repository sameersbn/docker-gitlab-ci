...
```bash
docker pull sameersbn/gitlab-ci:7.9.2
```
...
You need to provide the URL of the GitLab server while running GitLab CI using the `GITLAB_URL` environment configuration. Additionally, since version `7.9.2` you need to provide the `GITLAB_APP_ID` and `GITLAB_APP_SECRET`. For example if the location of the GitLab server is `172.17.0.2`,
...
```bash
docker run --name=gitlab-ci -d [OPTIONS] sameersbn/gitlab-ci:7.9.2
```
