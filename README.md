...
```bash
docker pull sameersbn/gitlab-ci:7.13.5
```
...
docker run --name gitlab-ci -it --rm [OPTIONS] \
    sameersbn/gitlab-ci:7.13.5 app:rake backup:create
```
...
```bash
docker run --name gitlab-ci -it --rm [OPTIONS] \
    sameersbn/gitlab-ci:7.13.5 app:rake backup:restore
```
...
```bash
docker run --name gitlab-ci -it --rm [OPTIONS] \
    sameersbn/gitlab-ci:7.13.5 app:rake backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab-ci -d [OPTIONS] sameersbn/gitlab-ci:7.13.5
```
